...
export const zhTW: UserGuide = {
    ...
    entries: [
      {
        term: '工作階段與子 AI 助手',
        detail:
          '每個工作階段都是擁有獨立脈絡的對話，支援按專案或全域匯總檢視。當任務產生子 AI 助手（Subagent）時，會以縮排卡片形式掛載在父對話下，複雜分工依然一清二楚。',
      },
      {
        term: '專案切換',
        detail:
          '隨時切換 AI 助手的工作目錄，或手動輸入路徑開啟新專案。選定專案會直接還原其最近一次的對話，無需每次從頭開啟新對話。',
      },
      {
        term: '模型與 AI 助手角色',
        detail:
      ...
      },
      {
        term: '附件發送與檔案提及',
        detail:
          '支援傳送相片、相簿圖片或任意檔案。圖片在傳送時會自動重新編碼，抹除 EXIF 等隱私中繼資料。在輸入框輸入 @ 可直接搜尋並引用專案中的檔案，省去手工描述的繁複。',
      },
      {
        term: '權限確認與問題回覆',
        detail:
          '當 AI 助手需要執行指令、修改檔案或讀取專案外的檔案時，會彈出確認卡片展示路徑與細節，提供「允許」、「一律允許」與「拒絕」三個選項——與系統通知按鈕文字完全一致，在鎖定螢幕上即可直接審批。AI 助手提問時會呈現為小表單供你填寫。亦可開啟「自動核准所有動作」以供觀察，但具破壞性的危險指令即便在此模式下依然會被強行攔截。',
      },
      {
        term: '背景工作與排隊執行',
        detail:
      ...
      },
      {
        term: '版本復原（Undo）',
        detail:
          '輸入 /undo 可將專案檔案狀態瞬間復原到你上一條訊息發出之前；輸入 /redo 可取消尚未定案的復原。特意設計為明確指令而非單鍵按鈕，杜絕意外誤觸。',
      },
    ],
  ...
  troubleshooting: {
    ...
    entries: [
      ...
      {
        term: 'AI 助手請求存取專案以外的路徑',
        detail:
          '提示「讀取專案外內容」或「寫入專案外內容」正如其字面意義，彈出的卡片上會清晰標示涉及的具體絕對路徑。點選「允許」授權本次，「一律允許」記住該路徑，「拒絕」則攔截本次動作。安全機制正在發揮防護作用——審批前請務必仔細核對目標路徑。',
      },
      {
