...
export const zhCN: UserGuide = {
    ...
    entries: [
      {
        term: '会话与子AI助手',
        detail:
          '每个会话都是拥有独立上下文的对话，支持按项目或全局汇总展示。当一个任务衍生出子AI助手（Subagent）时，会以缩进卡片形式挂载在父会话下，复杂分工脉络依然一目了然。',
      },
      {
        term: '项目切换',
        detail:
          '随时切换AI助手的工作目录，或手动输入路径开辟新项目。选定项目会直接恢复其最近一次的会话，无需每次从头开新对话。',
      },
      {
        term: '模型与AI助手身份',
        detail:
      ...
      },
      {
        term: '附件发送与文件引用',
        detail:
          '支持发送照片、相册图片或任意文件。图片在发送时会自动重新编码，抹除 EXIF 等隐私元数据。在输入框输入 @ 可直接模糊搜索并引用项目中的文件，省去手工描述的繁琐。',
      },
      {
        term: '权限确认与问题回答',
        detail:
          '当AI助手需要执行命令、修改文件或读取项目外的文件时，会弹出确认卡片展示路径与细节，提供“允许”、“始终允许”与“拒绝”三个选项——与系统通知按钮文字完全一致，在锁屏界面即可直接审批。AI助手提问时会呈现为小表单供你填写。也支持开启“自动批准所有操作”以供近距观察，但极具破坏性的危险命令即便在此模式下依然会被强行拦截。',
      },
      {
        term: '后台任务与排队执行',
        detail:
      ...
      },
      {
        term: '版本回滚（Undo）',
        detail:
          '输入 /undo 可将项目文件状态瞬时回滚到你上一条消息发出之前；输入 /redo 可取消尚未定稿的回滚。故意设计为显式命令而非单触按钮，杜绝意外误操作。',
      },
    ],
  ...
  troubleshooting: {
    ...
    entries: [
      ...
      {
        term: 'AI助手请求访问项目之外的路径',
        detail:
          '提示“读取项目外内容”或“写入项目外内容”正如其字面意思，弹出的卡片上会清晰标出涉及的具体绝对路径。点击“允许”授权本次，“始终允许”记住该路径，“拒绝”则拦截本次操作。安全防护机制正在尽职尽责——审批前请务必仔细核对目标路径。',
      },
      {
